                ...
                for batch in self.source.read_batch(self.settings.source.batch_size):
                    try:
                        with tracer.start_as_current_span("process_batch") as batch_span:
                            ...
                            # Write batch to sink
                            self.sink.write_batch(batch, partition_cols=self.settings.sink.partition_by)
                            
                            batch_count += 1
                            total_records += num_rows
                            
                            # Update metrics
                            batches_processed.add(1, {"source": self.settings.source.type, "sink": self.settings.sink.type})
